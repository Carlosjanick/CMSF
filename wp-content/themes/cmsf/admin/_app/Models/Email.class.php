...
class Email {
    ...
    public function Enviar(array $Data) {
        ...
        if (in_array('', $this->Data)):
            //$this->Error = ['Erro ao enviar mensagem: Para enviar esse e-mail. Preencha os campos requisitados!', WS_INFOR];
            $this->Error  = 'errempty';
            $this->Result = false;
        elseif (!Check::Email($this->Data['RemetenteEmail'])):
            //$this->Error = ['Erro ao enviar mensagem: O e-mail que você informou não tem um formato válido. Informe seu E-mail!', WS_ERROR];
            $this->Error  = 'errmail';
            $this->Result = false;
        else:
            $this->setMail();
    ...
    private function setMsg() {
        $this->Mensagem = "{$this->Mensagem}<hr><small>Recebida em: " . date('d/m/Y H:i') . "</small> <hr>
        Rodapé da Mensagem <hr>";
