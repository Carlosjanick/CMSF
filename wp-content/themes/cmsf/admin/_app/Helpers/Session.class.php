...
class Session {
    ...
    /*
     * ***************************************
     * *** Utilizador, VISITAS, ATUALIZAÇÕES ***
     * ***************************************
     */
    ...
    private function setTraffic() {
        ...
        if (!$this->Traffic):
            ...
            $createSiteViews->ExeCreate('fg_siteviews', $ArrSiteViews);
        else:
            if (!$this->getCookie()):
            ...
            $updateSiteViews->ExeUpdate('fg_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");
        endif;
    }
    ...
    private function TrafficUpdate() {
        ...
        $updatePageViews->ExeUpdate('fg_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function getTraffic() {
        ...
        $readSiteViews->ExeRead('fg_siteviews', "WHERE siteviews_date = :date", "date={$this->Date}");
        if ($readSiteViews->getRowCount()):
            $this->Traffic = $readSiteViews->getResult()[0];
        endif;
    }

    //Verifica, cria e atualiza o cookie do Utilizador [ HELPER TRAFFIC ]
    private function getCookie() {
        $Cookie = filter_input(INPUT_COOKIE, 'useronline', FILTER_DEFAULT);
        setcookie("useronline", base64_encode("us"), time() + 86400); //valido por 24 hora
        if (!$Cookie):
            return false;
        else:
            return true;
    ...
    //Identifica navegador do Utilizador!
    ...
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('fg_siteviews_agent', "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResult()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('fg_siteviews_agent', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResult()[0]['agent_views'] + 1];
            $updateAgent = new Update;
            $updateAgent->ExeUpdate('fg_siteviews_agent', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
    ...
    //Registra Utilizador online na tabela!
    private function setUsuario() {
        $sesOnline = $_SESSION['useronline'];
        $sesOnline['agent_name'] = $this->Browser;

        $userCreate = new Create;
        $userCreate->ExeCreate('fg_siteviews_online', $sesOnline);
    }

    //Atualiza navegação do Utilizador online!
    private function UsuarioUpdate() {
        ...
        $userUpdate->ExeUpdate('fg_siteviews_online', $ArrOnlone, "WHERE online_session = :ses", "ses={$_SESSION['useronline']['online_session']}");

        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('fg_siteviews_online', 'WHERE online_session = :onses', "onses={$_SESSION['useronline']['online_session']}");
