...
/**
 * Upload.class [ HELPER ]
 * Reponsável por executar upload de imagens, ficheiros e mídias no sistema!
...
class Upload {
    ...
    /**
     * <b>Enviar Ficheiro:</b> Basta envelopar um $_FILES de um ficheiro e caso queira um nome e um tamanho personalizado.
    ...
    public function File(array $File, $Name = null, $Folder = null, $MaxFileSize = null) {
        ...
        if ($this->File['size'] > ($MaxFileSize * (1024 * 1024))):
            $this->Result = false;
            $this->Error = "Ficheiro muito grande, tamanho máximo permitido de {$MaxFileSize}mb";
    ...
    /**
     * <b>Enviar Midia:</b> Basta envelopar um $_FILES de uma mídia e caso queira um nome e um tamanho personalizado.
    ...
    public function Media(array $Media, $Name = null, $Folder = null, $MaxFileSize = null) {
        ...
        elseif (!in_array($this->File['type'], $FileAccept)):
            $this->Result = false;
            $this->Error = 'Tipo de ficheiro não suportado. Envie audio MP3 ou vídeo MP4!';
        else:
            $this->CheckFolder($this->Folder);
            $this->setFileName();
            $this->MoveFile();
...
//Verifica e cria os diretórios com base em tipo de ficheiro, ano e mês!
    private function CheckFolder($Folder) {
        ...
        $this->Send = "{$Folder}/{$y}/{$m}/"; //armanzena caminho da uploads
    }
...
//Verifica e monta o nome dos ficheiros tratando a string!
...
//Realiza o upload de imagens redimensionando a mesma!
    private function UploadImage() {
        ...
        if (!$this->Image):
            $this->Result = false;
            $this->Error = 'Tipo de ficheiro inválido, envie imagens JPG ou PNG !';
        else:
...
//Dinamico para Enviar arquivos(doc pdf) e mídias (videos)
    public function MoveFile() {
        if (move_uploaded_file($this->File['tmp_name'], self::$BaseDir . $this->Send . $this->Name)):
            ...
            $this->Result = false;
            $this->Error = 'Erro ao mover o ficheiro. Favor tente mais tarde!';
        endif;
    }
